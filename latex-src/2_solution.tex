...
``$[100 \times r_\text{obs}]\%$ of tests passed. $[100 \times q]\%$ confidence that the true failure rate among datasets is at least $p_{\text{fail,lower}}$.''
...
Additionally, since $k_{\text{floor}} = -C \times \ln(1 - q)$, we can measure the effect of varying $C$ on $k_{\text{floor}}$ for a given q:
\begin{align*}
...
\begin{align}
    objective_{score}(p_s) = bias_{std}(p_s) - \beta \times compute_{frac}(p_s)
\end{align}
...
The result, shown in Figure \ref{fig:ps_hill_fit}, demonstrates a flat region of optimal $p_s$ values centered at $3.35 \times 10^{6}$, in which bias becomes relatively insensitive to $p_s$, while compute savings continue to improve significantly as $p_s$ decreases.

The tradeoff curve and suggested optimal value are shown in Figure \ref{fig:ps_experiment4}.

We therefore recommend a nominal value of $p_s = 3.35 \times 10^{6} \pm 1.98 \times 10^{4}$ for a good balance between compute and bias. Compared to the largest reasonable value of $p_s = 1e7$, this value results in only a $~3\%$ increase in bias stddev, while saving $~24\%$ of worst-case compute. By comparison, the less conservative value of $p_s = 4.43e5$ results in a $~12\%$ increase in bias stddev, while saving $~39\%$ of worst-case compute.
