...
\label{sec:ps_experiment1}
    ...
    \begin{figure}[ht!]
        ...
        \caption{Graph demonstrating the lack of correlation between $p\_\text{fail}$, $\text{ratio}\_\text{pass}$, and bias in the base case of $p_s = 1$}
        \label{fig:ps_experiment1}
    \end{figure}

    \paragraph{Objective.}
    To investigate the relationship between bias, $p_\text{fail}$, and pass ratio.
    $p_\text{fail}$ is calculated using $p$, $q$ and $n$, and so betwen it and $ratio\_pass$ we can explore the bias behavior of BIOD across the parameter space in the base-case of $p_s = 1$.

    \subsubsection{Setup.}

    \begin{itemize}
        \item We select $ratio_pass \in \{0.2, 0.5, 0.8, 0.9, 0.95, 0.99, 0.999\}$.
    ...
    \begin{itemize}
        ...
        \item Run BIOD using the failure rate found, noting the $pass\_ratio$ value at the end of the run, the initial estimate for $k$ and the number of iterations actually performed.
        \item Compute bias as $p_\text{fail, true} - pass\_ratio$.
        \item Compute the mean and standard deviation of bias across all iterations for each $ratio\_pass$ value.
    \end{itemize}
            
    For each $p_\text{fail}$, compute:
    \begin{itemize}
        \item The mean of all bias means across all $ratio\_pass$ values.
        \item The standard deviation of all bias means across all $ratio\_pass$ values.
        \item The mean of all bias standard deviations across all $ratio\_pass$ values.
        \item The standard deviation of all bias standard deviations across all $ratio\_pass$ values.
    \end{itemize}
    ...
    \subsubsection{Definition of Metrics.}
        ...
        \begin{itemize}
            ...
            \item $ratio\_pass$: Pass ratio parameter provided to BIOD.
    ...
    We expect bias to increase proportionally with the difference between $p_\text{fail}$ and $ratio\_pass$.

    \subsubsection{Results.}

    Contrary to our hypothesis, we find that bias remains relatively constant across the parameter space, with mean bias values clustering around $0.0$ and standard deviation of bias remaining low for all combinations of $p_\text{fail}$ and $ratio\_pass$.

    We also found that the standard deviation of bias mean and standard deviation of bias standard deviation across all $ratio\_pass$ values remained low for all $p_\text{fail}$ values.
...
\subsection{Experiment 7: Prior Strength Derivation} \label{sec:ps_experiment4}
    ...
    \begin{itemize}
        ...
        \[
            \text{objective}(p_s, \beta) = hill_{\text{bias}} - \beta \cdot hill_{\text{compute}}
        \]
        ...
        \[
            \text{error}(p_s) = \frac{\sigma hill_{\text{bias}}^2 + (\beta \cdot \sigma hill_{\text{compute}}^2) }{\frac{\partial \text{objective}(p_s, \beta) }{\partial p_s}}
        \]
    ...
    The table shown in Figure~\ref{fig:ps_hill_fit} shows a value of $p_s = 3.35 \times 10^{6} \pm 1.98 \times 10^{4}$ 
    ...
    We recommend a prior strength value of $p_s = 3.35 \times 10^{6} \pm 1.98 \times 10^{4}$ for BIOD, balancing bias reduction and computational cost effectively for most applications.
